...
function auto_add_tax_for_room( $cart ) {
    if ( is_admin() && ! defined('DOING_AJAX') ) return;
     ...
     foreach($items as $item => $values) { 
          ...
          if ( $attributes ) {
               if($attributes['pa_editions'] == 'pb' || $attributes['pa_editions'] == 'hb'){
                    //echo $attributes['pa_editions'];
                    $handle_fee = 'true';
               }
          }
     }
...
/****/
function sample_after_wc_tabs_panel_content() {
    global $product;
    $productId = get_the_ID();
    $sample_content = get_field("sample_content", $productId);
    if(!empty($sample_content)): ?>
     <h2 class="sampleText">Sample</h2>
     <?php echo $sample_content; ?>
  <?php endif;
}
add_action('bookworm_after_wc_tabs_panel_content', 'sample_after_wc_tabs_panel_content');